public class String1 {
    ...
    public String makeTags(String tag, String word) {
        String tagOpen = "<".concat(tag.concat(">"));
        String tagClose = "</".concat(tag.concat(">"));
        return tagOpen.concat(word.concat(tagClose));
    }
    ...
    public String extraEnd(String str) {
        int size = str.length();
        String suffix = str.substring(size - 2, size);
        return suffix.concat(suffix.concat(suffix));
    }
    ...
    public String firstHalf(String str) {
        int half = Math.round(str.length() / 2);
    ...
    public String nTwice(String str, int n) {
        int size = str.length();
        String preffix = str.substring(0, n);
        String suffix = str.substring(size - n, size);
    ...
    public String lastTwo(String str) {
        ...
        if(size == 2) {
            return str.substring(1, 2).concat(str.substring(0, 1));
        }
        return str.substring(0, size - 2)
                .concat(str.substring(size - 1, size)
                        .concat(str.substring(size -2, size - 1)));
    }
    ...
    public boolean frontAgain(String str) {
        int size = str.length();
        if(size >= 2 && str.substring(0,2).equals(str.substring(size - 2, size))) {
            return true;
        }
        return false;
    }
    ...
    public String minCat(String a, String b) {
        ...
        return a.substring(a.length() - minSize)
                .concat(b.substring(b.length() - minSize));
    }
    ...
    public String extraFront(String str) {
        ...
        return subString.concat(subString.concat(subString));
    }
    ...
    public String deFront(String str) {
        String buffer = str.substring(2);
        if(str.substring(1,2).equals("b")) {
            buffer = "b".concat(buffer);
        }
        if(str.substring(0,1).equals("a")) {
            buffer = "a".concat(buffer);
        }
        return buffer;
    }
    ...
    public String startWord(String str, String word) {
        ...
        int min = Math.min(sizeStr, sizeWord);
        String buffer = "";
        if(sizeStr == 0 ||
                (sizeStr == 1 && sizeWord != 1 && !str.substring(0, 1).equals(word.substring(0, 1)))) {
            return buffer;
        }
    ...
    public String withoutX(String str) {
        ...
        if (size == 0) {
            ...
            if(str.substring(0, 1).equals("x")) {
                buffer = str.substring(1, size - 1);
            } else {
                buffer = str.substring(0, size - 1);
            }
            if(!str.substring(size -1, size).equals("x")){
                buffer = buffer.concat(str.substring(size -1, size));
            } else {
                return buffer;
            }
        }
    ...
    public String withoutX2(String str) {
        ...
        if (size == 0) {
            ...
            if (!str.substring(0, 1).equals("x")) {
                buffer = buffer.concat(str.substring(0, 1));
            }
            if (!str.substring(1, 2).equals("x")) {
                buffer = buffer.concat(str.substring(1, 2));
            }
            if (size > 2) {
                buffer = buffer.concat(str.substring(2, size));
            }
        }
        return buffer;
    }
